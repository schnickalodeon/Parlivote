...
{
    [Fact]
    public async Task ShouldUpdateVoteViewAsyncAsync()
    {
        // Arrange
        ...
        this.voteServiceMock.Setup(service =>
            service.ModifyAsync(It.IsAny<Vote>()))
                .ReturnsAsync(returnedVote);

        // Act
        VoteView actualVoteView =
            await this.voteViewService.UpdateAsync(inputVoteView);
        ...
        this.voteServiceMock.Verify(service =>
            service.ModifyAsync(It.Is(SameVoteAs(expectedInputVote))),
            Times.Once);
    }
}